...
[
  ...
  {"section":"01 章节","title":"页标题","subtitle":"页副标题","layout":"two_column",
   "panels":[{"title":"栏1","body":"多行\n","fill":"light","border":"accent"},
             {"title":"栏2","body":[{"label":"L5 ...","color":"accent"}]}]}
]
```
- `layout` 默认 `default`（points 列表）；`two_column` 左右两栏
- `panels[i].body` 可为字符串（\n 多行）或列表（结构化项：label/color）
- `fill`/`border` 取色键：`light`/`accent`/`gray`/`title`/`bg`
...
- 固定坐标布局，要点超 6 个溢出（需手动分页；two_column 模式可放更多内容）
- 支持 `default` / `two_column` 两种布局；更复杂的表格/图表需扩展 `LAYOUT_RENDERERS`
