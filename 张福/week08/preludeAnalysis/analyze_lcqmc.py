...
def plot_class_distribution(results, output_dir):
    ...
    plt.close()
    print("类别分布图已保存到 " + os.path.join(output_dir, 'class_distribution.png'))
...
def plot_length_distribution(results, output_dir):
    ...
    plt.close()
    print("长度分布图已保存到 " + os.path.join(output_dir, 'length_distribution.png'))
...
def plot_comparison(results, output_dir):
    ...
    plt.close()
    print("对比图已保存到 " + os.path.join(output_dir, 'comparison.png'))

def generate_summary(results, output_dir):
    """生成统计摘要"""
    summary_lines = []
    summary_lines.append("=" * 80)
    summary_lines.append("LCQMC 数据集统计分析报告")
    summary_lines.append("=" * 80)
...
def main():
    # 设置路径
    base_dir = r'd:\workspace\hub-TroE\张福\week08\data\lcqmc'
    output_dir = r'd:\workspace\hub-TroE\张福\week08\preludeAnalysis\output'
    ...
    print("\n分析完成！所有结果已保存到 preludeAnalysis/output 目录")
