"""
人民日报 NER（peoples_daily）数据集探索与可视化

教学重点：
  1. BIO 标注格式的统计方法（B-PER/I-PER 等标签如何还原实体 span）
  2. 各实体类型的分布差异（PER/ORG/LOC 类别不均衡是 NER 的难点）
...
ENTITY_LABELS = {"PER": "人名", "ORG": "组织机构", "LOC": "地点"}
...
def print_summary(stats_train: dict, stats_val: dict):
    print("=" * 70)
    print("人民日报 NER（peoples_daily）数据集统计摘要")
    print("=" * 70)
    ...
    for etype, cnt in sorted(stats_train["entity_type_counts"].items(), key=lambda x: -x[1]):
        cn = ENTITY_LABELS.get(etype, etype)
        print(f"  {etype:5s} ({cn:8s}) : {cnt:5d} 条")
...
def plot_entity_distribution(stats_train: dict):
    ...
    labels = [f"{k}\n({ENTITY_LABELS.get(k, k)})" for k in sorted(counts)]
...
def plot_text_length_distribution(stats_train: dict):
    ...
    ax.set_title("人民日报 NER 文本长度分布（训练集）", fontsize=14)
    ax.set_xlabel("文本字符数")
    ax.set_ylabel("样本数")
    ax.legend()
    plt.tight_layout()
...
def parse_args():
    parser = argparse.ArgumentParser(description="探索人民日报 NER（peoples_daily）数据集")
