...
def load_model(model_path, ckpt_dir, device):
...
def gold_spans_from_record(record):
    ...
    for i, tag in enumerate(ner_tags):
        if tag == "O":
            if entity_start_idx != -1:
                text = "".join(tokens[entity_start_idx:i])
                spans.add((text, entity_type, entity_start_idx, i))
                entity_start_idx = -1
                entity_type = None
    ...
    if entity_start_idx != -1:
        text = "".join(tokens[entity_start_idx:len(tokens)])
        spans.add((text, entity_type, entity_start_idx, len(tokens)))
    return spans


def generate_ner(text, model, tokenizer, device, max_new_tokens: int = 256):
...
def pred_spans_from_output(text, raw_output):
    ...
    for ent in entities:
        ...
        if idx == -1:
            continue
        spans.add((surface, etype, idx, idx + len(surface)))
    return spans


def compute_span_f1(all_golds: list[set], all_preds: list[set]):
...
def main():
    ...
    if not ckpt_dir.exists():
        raise ValueError(f"checkpoint 目录 {ckpt_dir} 不存在")
