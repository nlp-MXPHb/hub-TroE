...
使用方式：
  ...
  python explore_data.py --data_dir ../data/lcqmc --output_dir ../outputs/figures
...
def _get_font():
    global _CN_FONT
    if _CN_FONT is None:
        # 按字体名查找（比搜文件路径更可靠，避免匹配到非中文的 NotoSans* 字体）
        installed = {f.name for f in fm.fontManager.ttflist}
        preferred = [
            "STHeiti", "Songti SC", "PingFang HK", "Heiti TC",   # macOS
            "SimHei", "Microsoft YaHei", "SimSun",               # Windows
            "Noto Sans CJK SC", "Noto Sans SC", "WenQuanYi Micro Hei",  # Linux
        ]
        font_name = next((n for n in preferred if n in installed), None)
        if font_name:
